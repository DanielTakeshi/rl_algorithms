...
axarr[0].set_title("Pong Performance (Blocks of 100)", fontsize=title_size)
axarr[0].plot(pong_t, pong_mean, lw=lw)
axarr[1].set_title("Pong Performance (Best 100 Block)", fontsize=title_size)
axarr[1].plot(pong_t, pong_best_mean, lw=lw)

for i in range(2):
    axarr[i].set_xlabel("Episodes", fontsize=axis_size)
    axarr[i].set_ylabel("Rewards", fontsize=axis_size)
    axarr[i].set_xlim([0,max(pong_t)])
    axarr[i].set_ylim([-21,21])
